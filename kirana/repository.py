...
class KiranaRepository:
    ...
    def get_customer_price_memory(self, store_id: int, customer_id: int) -> list[dict]:
        ...
        with self._conn() as conn:
            rows = conn.execute(
                text(sql), {"sid": store_id, "cid": customer_id}
            ).mappings().all()
        return [dict(r) for r in rows]

    def set_customer_product_price(self, store_id: int, customer_id: int,
                                   product_id: int, price: float | None) -> dict:
        ...
        with self._conn() as conn:
            ...
            """), {"sid": store_id, "cid": customer_id, "pid": product_id, "price": price})
            conn.commit()
    ...
    def get_basket(self, store_id: int, basket_id: int) -> dict | None:
        ...
        """
        with self._conn() as conn:
            row = conn.execute(text(sql), {"sid": store_id, "bid": basket_id}).mappings().first()
        return dict(row) if row else None
    ...
    def _gross_total_for_items(self, conn, store_id: int, items: list[dict]) -> float:
        ...
        for item in items:
            ...
            price_row = conn.execute(text("""
                ...
                ORDER BY valid_from DESC LIMIT 1
            """), {"pid": pid, "sid": store_id}).mappings().first()
            unit = float(price_row["price"]) if price_row and price_row["price"] is not None else 0.0
            gross += unit * qty
        return gross
    ...
    def create_basket(self, store_id: int, data: dict) -> dict:
        ...
        with self._conn() as conn:
            ...
            row = conn.execute(text("""
                ...
                RETURNING basket_id
            """), {
                "sid": store_id, "name": data["name"], "desc": data.get("description"),
                "price": pricing["price"], "tier": pricing["tier"],
                "gross": pricing["gross_total"], "disc": pricing["discount_pct"],
            ...
            if items:
                ...
                """), [{"bid": basket_id, "pid": item["product_id"],
                       ...
                       for item in items])
            conn.commit()
        return self.get_basket(store_id, basket_id) or {"basket_id": basket_id, **pricing}
    ...
    def update_basket(self, store_id: int, basket_id: int, data: dict) -> dict | None:
        ...
        with self._conn() as conn:
            ...
            ), {"bid": basket_id, "sid": store_id}).first()
            if not owns:
                return None
            items = data.get("items", [])
            gross = self._gross_total_for_items(conn, store_id, items)
            ...
            """), {
                "bid": basket_id, "sid": store_id, "name": data["name"],
                "desc": data.get("description"), "price": pricing["price"],
                "tier": pricing["tier"], "gross": pricing["gross_total"],
                "disc": pricing["discount_pct"], "vf": data.get("valid_from"),
                "vt": data.get("valid_to"),
            })
            conn.execute(text("DELETE FROM kirana_oltp.basket_item WHERE basket_id = :bid"),
                         {"bid": basket_id})
            if items:
                conn.execute(text("""
                    INSERT INTO kirana_oltp.basket_item(basket_id, product_id, product_name, qty)
    ...
    def retier_baskets(self, store_id: int) -> int:
        ...
        config = self.get_tier_config(store_id)
        with self._conn() as conn:
            ids = [r["basket_id"] for r in conn.execute(text(
            ...
            for bid in ids:
                ...
                gross = self._gross_total_for_items(conn, store_id, [dict(i) for i in items])
                pricing = price_for(gross, config)
                conn.execute(text("""
                    UPDATE kirana_oltp.basket SET price = :price, tier = :tier,
                        gross_total = :gross, discount_pct = :disc
                    WHERE basket_id = :bid
                """), {"bid": bid, "price": pricing["price"], "tier": pricing["tier"],
                       "gross": pricing["gross_total"], "disc": pricing["discount_pct"]})
            conn.commit()
        return len(ids)
    ...
    def set_basket_archived(self, store_id: int, basket_id: int, archived: bool) -> bool:
        with self._conn() as conn:
    ...
    def basket_alerted_today(self, store_id: int, basket_id: int) -> bool:
        with self._conn() as conn:
            ...
            """), {"bid": basket_id, "sid": store_id}).mappings().first()
